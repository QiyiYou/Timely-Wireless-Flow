classdef DownlinkAPInstance < handle
    ...
    methods
        ...
        %get the next expiration slot for flow k if the current slot is t
        function [slot] = getNextExpirationSlot(obj, k, t)
            if( t <= 0 )
                error('wrong input');
            end
            % judge which packets will expire in the next expiration slot 
            period_ind = ceil((t-obj.offset(k)-obj.delay(k))/obj.period(k));
            slot = obj.offset(k) + obj.delay(k) + obj.period(k)*period_ind;
        end
