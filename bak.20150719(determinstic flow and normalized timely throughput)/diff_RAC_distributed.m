...
% end
% obj.success_prob = randi([10,99],obj.n_flow,1)/100; %the probability to successfully deliver a packet of each flow if scheduled, let it larger than 0.1
...
tic;
[Rec_theoretical_utlity, Rec_theoretical_throughput, Rec_empirical_utility_distributed, Rec_empirical_throughput_per_flow_distributed, state_action_distribution_distributed] ...
...
plot(1:step:T,Rec_empirical_utility_distributed(1:step:T), '--k', 'Linewidth', 3);
plot(1:step:T,Rec_theoretical_utlity(1:step:T), '-.b', 'Linewidth', 3);
hold off;
...
%ylim([-1.5,-0.5]);
legend('Optimal RAC', 'Distributed-Empirical', 'Distributed-Theoretical', 'location', 'NorthEast');
box on;
grid on;
...
for kk=1:obj.n_flow
    ...
    plot(1:step:T, Rec_empirical_throughput_per_flow_distributed(kk,1:step:T), '--k', 'Linewidth', 3);
    plot(1:step:T, Rec_theoretical_throughput(kk,1:step:T), '-.b', 'Linewidth', 3);
    hold off;
    xlabel('Slot','FontSize', 20, 'FontName', 'Arial');
    ...
    if(optimal_throughput_per_flow_RAC(kk) <= 0.5)
        ...
        legend('Optimal RAC', 'Distributed-Empirical', 'Distributed-Theoretical', 'location', 'SouthEast');
    end
    ylim([0,1]);
    box on;
    grid on;
