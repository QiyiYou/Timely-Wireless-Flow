...
obj.n_flow = 5; %number of users/flows
obj.offset = randi([1,10], obj.n_flow,1); %the offset of the first packet of each flow
obj.period = randi([1,10], obj.n_flow,1); %the period of each flow
obj.delay = zeros(obj.n_flow,1); %the delay of each flow, delay(ii) <= period (ii) because we only consider non-overlapping traffic
for ii=1:obj.n_flow
...
tic;
[virtual_arrival_EDF, virtual_server_capacity_EDF, virtual_departure_EDF, virtual_queue_EDF, state_action_distribution_EDF, ...
 Rec_system_state_EDF, Rec_system_action_EDF, Rec_state_action_per_slot_EDF] = ...
    EDFlargestQueueFirstScheduling(obj, T_lqfs, strict_throughput_per_flow);
fprintf('Finish EDFLQFS with time %f seconds\n', toc);

avg_virtual_arrival_EDF = mean(virtual_arrival_EDF,2)
avg_virtual_server_capacity_EDF = mean(virtual_server_capacity_EDF,2)
avg_virtual_departure_EDF = mean(virtual_departure_EDF,2)
avg_empirical_throughput_EDF = avg_virtual_departure_EDF.*obj.period
final_virtual_queue_EDF = virtual_queue_EDF(:,end)
ratio_final_virtual_queue_to_time_EDF = virtual_queue_EDF(:,end)/(T_lqfs)

cum_virtual_arrival_EDF = cumsum(virtual_arrival_EDF,2);
cum_virtual_server_capacity_EDF = cumsum(virtual_server_capacity_EDF,2);
cum_virtual_departure_EDF = cumsum(virtual_departure_EDF,2);

for nn=1:obj.n_flow
    figure;
    set(gca,'FontSize',20);
    hold on;
    plot(1:T_lqfs,cum_virtual_arrival_EDF(nn,:), '-b', 'Linewidth', 3);
    plot(1:T_lqfs,cum_virtual_server_capacity_EDF(nn,:), '-g', 'Linewidth', 3);
    plot(1:T_lqfs,cum_virtual_departure_EDF(nn,:), '--r', 'Linewidth', 2);
    plot(1:T_lqfs,virtual_queue_EDF(nn,:), '-k', 'Linewidth', 3);
    hold off;
    xlabel('Slot','FontSize', 20, 'FontName', 'Arial');
    legend('Cumulative Arrival', 'Cumulative Server Capacity', 'Cumulative Departure', 'Virtual Queue', 'Location', 'NorthWest');
    title(sprintf('%d flows, epsilon=%f',obj.n_flow, epsilon));
    box on;
    grid on;
    print(sprintf('stability_EDFLQFS_result_flow_%d',nn),'-dpdf');
end


%analyze the state and/or action evolution of EDFLQFS
Rec_system_state_distribution_EDF = zeros(T_lqfs, obj.n_state);
Rec_state_action_distribution_EDF = cummean(Rec_state_action_per_slot_EDF,3);

for ss=1:obj.n_state
    Rec_system_state_distribution_EDF(:, ss) = cummean(Rec_system_state_EDF == ss);
end


x_avg_RAC = squeeze(mean(sum(optimal_policy_RAC,3),1))
x_avg_EDFLQFS = squeeze(Rec_system_state_distribution_EDF(end,:))
y_avg_RAC = squeeze(mean(optimal_policy_RAC,1))
y_avg_EDFLQFS = squeeze(Rec_state_action_distribution_EDF(:,:,end))

diff_x_avg_RAC_EDFLQFS = sum(abs(Rec_system_state_distribution_EDF - ones(T_lqfs, 1)*x_avg_RAC),2);
diff_y_avg_RAC_EDFLQFS = sum(abs(reshape(y_avg_RAC, [], 1)*ones(1,T_lqfs) - reshape(Rec_state_action_distribution_EDF, [], T_lqfs)),1);

figure;
set(gca,'FontSize',20);
hold on;
plot(1:T_lqfs,diff_x_avg_RAC_EDFLQFS, '-b', 'Linewidth', 3);
hold off;
xlabel('Slot','FontSize', 20, 'FontName', 'Arial');
ylabel('Diff x','FontSize', 20, 'FontName', 'Arial');
title(sprintf('%d flows, epsilon=%f',obj.n_flow, epsilon));
box on;
grid on;
print(sprintf('stability_EDFLQFS_result_diff_x'),'-dpdf');

figure;
set(gca,'FontSize',20);
hold on;
plot(1:T_lqfs,diff_y_avg_RAC_EDFLQFS, '-b', 'Linewidth', 3);
hold off;
xlabel('Slot','FontSize', 20, 'FontName', 'Arial');
ylabel('Diff y','FontSize', 20, 'FontName', 'Arial');
title(sprintf('%d flows, epsilon=%f',obj.n_flow, epsilon));
box on;
grid on;
print(sprintf('stability_EDFLQFS_result_diff_y'),'-dpdf');
