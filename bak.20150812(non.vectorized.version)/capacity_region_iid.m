...
save(sprintf('%s/capacity_region_iid.mat',filePath));

%load('fig/capacity_region/iid/n_flow=2/capacity_region_iid.mat');
...
%xlim([0,max_rate+0.1]);
%ylim([0,max_rate+0.1]);
legend('RAC','RAC-Approx','Location','Southwest');
box on;
grid on;
%print(sprintf('%s/capacity_regin_nonsyn', filePath),'-dpdf');
%print(sprintf('%s/capacity_regin_nonsyn', filePath),'-depsc');
export_fig(sprintf('%s/capacity_regin_iid', filePath), '-pdf','-transparent','-nocrop');
export_fig(sprintf('%s/capacity_regin_iid', filePath), '-eps','-transparent','-nocrop');
