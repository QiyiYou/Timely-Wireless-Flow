...
filePath = sprintf('fig/capacity_region/fig1_infocom/n_flow=%d',n_flow);
fileID = fopen(sprintf('%s/conf_capacity_region_fig1_infocom.txt',filePath),'w');
...
save(sprintf('%s/capacity_region_fig1_infocom.mat',filePath));
fclose(fileID);

load('fig/capacity_region/fig1_infocom/n_flow=2/capacity_region_fig1_infocom.mat');
...
xlim([0.2,0.4]);
ylim([0.2,0.25]);
legend('RAC','RAC-Approx','Location','Northeast');
box on;
grid on;
%print(sprintf('%s/capacity_regin_nonsyn', filePath),'-dpdf');
%print(sprintf('%s/capacity_regin_nonsyn', filePath),'-depsc');
export_fig(sprintf('%s/capacity_regin_fig1_infocom', filePath), '-pdf','-transparent','-nocrop');
export_fig(sprintf('%s/capacity_regin_fig1_infocom', filePath), '-eps','-transparent','-nocrop');
