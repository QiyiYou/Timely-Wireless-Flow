function [r0_opt, status] = getApproximateSolutionRAC_given_direction(obj, lambda)
...
cvx_begin
    ...
    variable z(obj.n_flow, obj.period_lcm, max_n_state, obj.n_action);

    variable z_action(obj.period_lcm, obj.n_action);
    ...
    for tt=1:obj.period_lcm
        sum(squeeze(z_action(tt,:))) == 1;
        for kk=1:obj.n_flow
            n_state = obj.flow_array{kk}.n_state;
%             for aa=1:obj.n_action
%                 sum(squeeze(z(kk,tt,1:n_state,aa))) == z_action(tt,aa);
%             end
            sum(squeeze(z(kk,tt,1:n_state,:)), 1) == z_action(tt,:);
        end
    end
            ...
            for tt=1:obj.period_lcm
                for ss=1:obj.flow_array{kk}.n_state
                    ...
                    for last_ss=1:obj.flow_array{kk}.n_state
                        %for last_action == kk
                        prob_temp = prob_temp + obj.flow_array{kk}.getTransitionProbability(tt+obj.period_lcm,last_ss,1, ss)*z(kk, tt,last_ss,kk);
                        
                        %for last_action != kk
                        prob_temp = prob_temp + obj.flow_array{kk}.getTransitionProbability(tt+obj.period_lcm,last_ss,2, ss)*(sum(z(kk, tt,last_ss,:)) - z(kk, tt,last_ss,kk));
...
%                         end
        ...
        for kk=1:obj.n_flow
            ...
            for tt=1:obj.period_lcm
                for ss=1:obj.flow_array{kk}.n_state
                    %for aa == kk
                    reward_temp = reward_temp+ obj.flow_array{kk}.reward_per_state_per_action(ss, 1)*z(kk,tt,ss,kk)/obj.period_lcm;
                    
                    %for aa != kk
                    reward_temp = reward_temp + obj.flow_array{kk}.reward_per_state_per_action(ss, 2)*(sum(z(kk,tt,ss,:)) - z(kk,tt,ss,kk))/obj.period_lcm;
