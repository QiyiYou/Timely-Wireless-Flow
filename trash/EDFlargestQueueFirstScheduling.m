function [virtual_arrival, virtual_server_capacity, virtual_departure, virtual_queue, ...
    ...
    = EDFlargestQueueFirstScheduling(obj, T, strict_throughput_per_flow)
% the EDF+largest-queue-first-scheduling algorithm
...
%         end
        ...
        %how to incorporate the deadline, we use the reciprocal of leading time to
        %normarlize the queue lenght
        % the smaller the leanding time, the more urgent of this flow
        next_expiration_slot = getNextExpirationSlot(obj, qq, tt);
        leading_time = (next_expiration_slot-tt+1);

        %normarlized timely throughput
        if( (1/leading_time)*b_packet*virtual_queue(qq,tt)*obj.success_prob(qq)/(obj.period_lcm/obj.period(qq))> largest_queue_length)
            largest_queue_idx = qq;
