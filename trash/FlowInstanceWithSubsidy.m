classdef FlowInstanceWithSubsidy < handle
    ...
    methods
        ...
        function  obj=FlowInstanceWithSubsidy()
        ...
        function constructRewardPerStatePerAction(obj)
            ...
            obj.reward_per_state_per_action(2:end,1) = obj.success_prob - obj.W;
                ...
                if(obj.hasPacket(state) ~= 1) %do not have any packet
                    ...
                    if(transmission_probability_temp < obj.success_prob) % the tranmission is successful with prob success_prob
                        ...
                        if(t >= obj.offset)
                            ...
                            if( mod(t - obj.offset, obj.period) == 0 && arrival_prob_temp < obj.arrival_prob) %next slot arrival prob!!
