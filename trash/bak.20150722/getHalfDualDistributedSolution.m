function [theoretical_utlity, theoretical_throughput, empirical_utility, empirical_throughput, primal_state_action_distribution] ...
    = getHalfDualDistributedSolution(obj, utility_coeff, utility_form, T)
...
for tt=1:T
    ...
    for ii=1:obj.period_lcm
        max_temp_val = -inf;
        temp_val = 0;
        primal_optimal_state(ii) = system_state(tt); % we select the optimal state as the current state
