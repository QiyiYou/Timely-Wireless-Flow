            ...
            for tt=1:obj.period
                for ss=1:obj.n_state
                    ...
                    if(ss_bin(end) == 1)
                        ...
                        if(tt ~= obj.period)
                            obj.transition_matrix_a(tt, ss, next_state_noarrival) = 1-obj.arrival_prob(tt+1); %Note that next slot arrival prob!!!
                            obj.transition_matrix_a(tt, ss, next_state_arrival) = obj.arrival_prob(tt+1);
                        else % tt == obj.period
                            obj.transition_matrix_a(tt, ss, next_state_noarrival) = 1-obj.arrival_prob(1); %Note that next slot arrival prob!!!
                            obj.transition_matrix_a(tt, ss, next_state_arrival) = obj.arrival_prob(1);
                        end
                    end
                end
            end
