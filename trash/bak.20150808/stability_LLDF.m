...
[successful_transmission, virtual_arrival, virtual_server_capacity, virtual_departure, virtual_queue, state_action_distribution, ...
 Rec_system_state, Rec_system_action, Rec_state_action_per_slot] = ...
    LLDF(obj, T_lqfs, strict_throughput_per_flow);
fprintf('Finish L-LDF with time %f seconds\n', toc);
...
for nn=1:obj.n_flow
    ...
    box on;
    grid on;
    print(sprintf('stability_L-LDF_result_flow_%d',nn),'-dpdf');
    print(sprintf('stability_L-LDF_result_flow_%d',nn),'-depsc');
end
