function [theoretical_utlity, theoretical_throughput, empirical_utility, empirical_throughput, primal_state_action_distribution] ...
    = getFullDualDistributedSolution2(obj, utility_coeff, utility_form, T)
...
%     system_state(tt+1) = next_state;
    ...
    for kk=1:obj.n_flow
        ...
        for ii=1:obj.period_lcm
            for ss=1:obj.n_state
                aa = kk;
                change_temp = change_temp +  primal_state_action_distribution(ii,ss,aa,tt)*obj.reward_per_state_per_action(ss,aa)/(obj.period_lcm/obj.period(kk));
            end
        end
    ...
    for ii=1:obj.period_lcm-1
        for ss=1:obj.n_state
            for last_ss = 1:obj.n_state
                for last_aa = 1:obj.n_action
                    change_temp = change_temp + obj.transition_matrix(ii, last_ss,last_aa,ss)*primal_state_action_distribution(ii,last_ss, last_aa,tt);
                end
            end
            for aa=1:obj.n_action
                change_temp = change_temp - primal_state_action_distribution(ii+1,ss, aa,tt);
            end
            %%...
    ...
    for ss=1:obj.n_state
        for last_ss = 1:obj.n_state
            for last_aa = 1:obj.n_action
                change_temp = change_temp + obj.transition_matrix(obj.period_lcm, last_ss,last_aa,ss)*primal_state_action_distribution(obj.period_lcm,last_ss, last_aa,tt);
            end
        end
        for aa=1:obj.n_action
            change_temp = change_temp - primal_state_action_distribution(1,ss, aa,tt);
        end
        %%...
